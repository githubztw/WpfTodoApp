...
public class TodoManager : ManagerBase
{
    ...
    public TodoItemDto CreateTodo(int userId, string title, string? description,
        DateTime? dueDate, TodoPriority priority)
    {
        if (string.IsNullOrWhiteSpace(title))
            throw new ArgumentException("任务标题不能为空");
        return _todoService.CreateTodo(userId, title, description, dueDate, priority);
    }

    public TodoItemDto UpdateTodo(int todoId, string title, string? description,
        DateTime? dueDate, TodoPriority priority)
    {
        if (string.IsNullOrWhiteSpace(title))
            throw new ArgumentException("任务标题不能为空");
        return _todoService.UpdateTodo(todoId, title, description, dueDate, priority);
    }
